...
const downloadContentFromMessage = async ({ mediaKey, directPath, url }, type, opts = {}) => {
    if (type === 'sticker-pack') {
        ...
        const fileEncSha256Base64 = Buffer.from(mediaKey).toString('base64').replace(/\+/g, '-').replace(/\//g, '_').replace(/=/g, '')
        const downloadUrl = url || getUrlFromDirectPath(directPath)
        const finalUrl = `${downloadUrl}?token=${fileEncSha256Base64}`
        ...
        const computedHmac = Crypto.createHmac('sha256', keys.macKey)
        computedHmac.update(keys.iv)
