...
const hkdfInfoKey = (type) => {
    const hkdfInfo = Defaults_1.MEDIA_HKDF_KEY_MAPPING[type] || `sticker-pack`
...
async function getMediaKeys(buffer, mediaType) {
    ...
    if (mediaType === 'sticker-pack') {
        return deriveStickerPackKeys(buffer)
    }
...
function deriveStickerPackKeys(mediaKey) {
    const salt = Buffer.alloc(32)
    const info = Buffer.from('WhatsApp Sticker Pack Keys')
    
    const hkdf = Crypto.createHmac('sha256', salt)
    const prk = hkdf.update(mediaKey).digest()
    
    const t = Buffer.alloc(0)
    const okm = Buffer.alloc(112)
...
const downloadContentFromMessage = async ({ mediaKey, directPath, url }, type, opts = {}) => {
    ...
    if (type === 'sticker-pack') {
        return downloadStickerPackContent(downloadUrl, mediaKey, opts)
    }
...
const downloadStickerPackContent = async (downloadUrl, mediaKey, { startByte, endByte, options } = {}) => {
    let bytesFetched = 0
    let startChunk = 0
    ...
    const { cipherKey, iv } = deriveStickerPackKeys(mediaKey)
...
module.exports = {
  ...
  deriveStickerPackKeys,
  downloadStickerPackContent
}
