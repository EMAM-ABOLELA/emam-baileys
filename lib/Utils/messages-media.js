...
const downloadContentFromMessage = async (message, type, opts = {}) => {
    const media = Object.values(message)[0]
    if (!media) {
        throw new boom_1.Boom('No media message found')
    }

    if (type === 'sticker-pack') {
        const directPath = media.directPath
        const mediaKey = media.mediaKey
        const fileEncSha256 = media.fileEncSha256
        
        const fileEncSha256Base64 = Buffer.from(fileEncSha256).toString('base64').replace(/\+/g, '-').replace(/\//g, '_').replace(/=/g, '')
        const url = `https://${DEF_HOST}${directPath}?token=${fileEncSha256Base64}`
        ...
        if (encryptedData.length <= MAC_SIZE) {
            throw new Error('Downloaded file is too short to contain MAC')
        }
        
        const ciphertext = encryptedData.slice(0, encryptedData.length - MAC_SIZE)
        const receivedMac = encryptedData.slice(encryptedData.length - MAC_SIZE)
        
        const computedHmac = Crypto.createHmac('sha256', macKey)
        computedHmac.update(iv)
        computedHmac.update(ciphertext)
        const computedMacFull = computedHmac.digest()
        const computedMac = computedMacFull.slice(0, MAC_SIZE)
        ...
        if (padLength > 0 && padLength <= BLOCK && padLength <= decrypted.length) {
            const padding = decrypted.slice(decrypted.length - padLength)
            if (padding.every(byte => byte === padLength)) {
        ...
        const downloadUrl = media.url || getUrlFromDirectPath(media.directPath)
        const keys = await getMediaKeys(media.mediaKey, type)
